...
namespace sm {
    ...
    bool Input::is_mouse_button_pressed(MouseButton button) {
        return glfwGetMouseButton(static_cast<GLFWwindow*>(window_handle), static_cast<int>(button)) == GLFW_PRESS;
    }

    float Input::get_mouse_x() {
        double x, y;
        glfwGetCursorPos(static_cast<GLFWwindow*>(window_handle), &x, &y);
