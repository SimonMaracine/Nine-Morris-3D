...
void JumpPlusVariantScene::on_start() {
    ...
    ctx.connect_event<sm::KeyReleasedEvent, &JumpPlusVariantScene::on_key_released>(this);
...
BoardObj& JumpPlusVariantScene::get_board() {
    return m_board;
}

void JumpPlusVariantScene::play_move_on_board(const std::string& string) {
    // TODO
}
...
JumpPlusVariantBoard JumpPlusVariantScene::setup_renderables() {
    ...
    return JumpPlusVariantBoard(
        ...
        [this](const JumpPlusVariantBoard::Move&) {
            if (!m_game_started) {
                m_timer.start();
