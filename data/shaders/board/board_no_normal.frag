...
vec3 calculate_light(
    ...
    const float specular_strength = pow(max(dot(view_direction, reflect_direction), 0.0), material.shininess);
    const vec3 specular_light = material.specular * specular_strength * light_specular;

    // Calculate shadow and final result
    const float shadow = calculate_shadow(fragment_position_light_space, normalized_normal, light_direction, u_shadow_map);
...
void main() {
    ...
    const vec3 total_light = calculate_light(
        u_material, v_light_position, u_light_ambient, u_light_diffuse,
        u_light_specular, v_view_position, texture_colors, v_fragment_position,
        v_fragment_position_light_space, v_normal
    );

    fragment_color = vec4(total_light, 1.0);
}
