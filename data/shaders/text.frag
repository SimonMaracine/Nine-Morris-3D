...
void main() {
    ...
    float dist2 = 1.0 - texture(u_bitmap, v_texture_coordinate + u_offset).r;
    float outline_alpha = 1.0 - smoothstep(u_border_width, u_border_width + border_edge, dist2);

    float overall_alpha = alpha + (1.0 - alpha) * outline_alpha;
    vec3 overall_color = mix(outline_color, u_color, alpha / overall_alpha);
