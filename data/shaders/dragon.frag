...
vec3 calculate_light() {
    // Ambient light
    const vec3 ambient_light = u_light_ambient;
    ...
    const vec3 diffuse_light = u_light_diffuse * diffuse_strength;
    ...
    const float specular_strength = pow(max(dot(view_direction, reflection), 0.0), 32);
    const vec3 specular_light = u_light_specular * specular_strength * 0.5;

    // All together
    const vec3 result = (ambient_light + diffuse_light + specular_light) * u_color;
