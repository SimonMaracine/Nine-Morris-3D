...
void UpdatePiecesSystem::run() {
    ...
    for (auto entity : view) {
        ...
        if (piece_c.selected) {
            ...
            model_c.material->set_vec3("u_material.tint", glm::vec3(1.0f, 0.2f, 0.2f));
        } else {
            model_c.material->set_vec3("u_material.tint", glm::vec3(1.0f, 1.0f, 1.0f));
        }

        if (piece_c.selected || piece_c.show_outline && hover_c.id == hovered_id && piece_c.in_use && !piece_c.pending_remove) {
            // FIXME maybe use registry.patch + on_update, or just use outline_enabled

            // if (!outline_c.outline_enabled) {
            //     app->renderer->update_model(model_c.model, Renderer::WithOutline | Renderer::CastShadow);
