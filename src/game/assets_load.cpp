...
namespace assets_load {
    void all_start(CustomLoader& loader, bool normal_mapping, const std::string& texture_quality, bool labeled_board, const std::string& skybox) {
        ...
        if (texture_quality == launcher_options::NORMAL) {
            loader().texture_data.load("board_wood_diff_texture"_h, encr(path_for_assets(BOARD_WOOD_DIFFUSE_TEXTURE)), true);
            loader().texture_data.load("white_piece_diff_texture"_h, encr(path_for_assets(WHITE_PIECE_DIFFUSE_TEXTURE)), true);
            loader().texture_data.load("black_piece_diff_texture"_h, encr(path_for_assets(BLACK_PIECE_DIFFUSE_TEXTURE)), true);

            if (normal_mapping) {
                loader().texture_data.load("board_norm_texture"_h, encr(path_for_assets(BOARD_NORMAL_TEXTURE)), true);
                loader().texture_data.load("piece_norm_texture"_h, encr(path_for_assets(PIECE_NORMAL_TEXTURE)), true);
            }

            if (labeled_board) {
                loader().texture_data.load("board_paint_diff_texture"_h, encr(path_for_assets(LABELED_BOARD_PAINT_DIFFUSE_TEXTURE)), true);
            } else {
                loader().texture_data.load("board_paint_diff_texture"_h, encr(path_for_assets(BOARD_PAINT_DIFFUSE_TEXTURE)), true);
            }
            ...
            loader().texture_data.load("board_wood_diff_texture"_h, encr(path_for_assets(BOARD_WOOD_DIFFUSE_TEXTURE_SMALL)), true);
            loader().texture_data.load("white_piece_diff_texture"_h, encr(path_for_assets(WHITE_PIECE_DIFFUSE_TEXTURE_SMALL)), true);
            loader().texture_data.load("black_piece_diff_texture"_h, encr(path_for_assets(BLACK_PIECE_DIFFUSE_TEXTURE_SMALL)), true);

            if (normal_mapping) {
                loader().texture_data.load("board_norm_texture"_h, encr(path_for_assets(BOARD_NORMAL_TEXTURE_SMALL)), true);
                loader().texture_data.load("piece_norm_texture"_h, encr(path_for_assets(PIECE_NORMAL_TEXTURE_SMALL)), true);
            }

            if (labeled_board) {
                loader().texture_data.load("board_paint_diff_texture"_h, encr(path_for_assets(LABELED_BOARD_PAINT_DIFFUSE_TEXTURE_SMALL)), true);
            } else {
                loader().texture_data.load("board_paint_diff_texture"_h, encr(path_for_assets(BOARD_PAINT_DIFFUSE_TEXTURE_SMALL)), true);
            }
