...
namespace assets_load {
    void load_assets(Loader<AssetsLoad>* loader) {
        ...
        loader->get()->board_texture = std::make_shared<TextureData>(path(BOARD_TEXTURE), true);
        loader->get()->board_paint_diffuse_texture = std::make_shared<TextureData>(path(BOARD_PAINT_TEXTURE), true);
        loader->get()->white_piece_texture = std::make_shared<TextureData>(path(WHITE_PIECE_TEXTURE), true);
        loader->get()->black_piece_texture = std::make_shared<TextureData>(path(BLACK_PIECE_TEXTURE), true);
    ...
    void load_assets_low_tex(Loader<AssetsLoad>* loader) {
        ...
        loader->get()->white_piece_mesh = load_model(path(WHITE_PIECE_MESH));
        loader->get()->black_piece_mesh = load_model(path(BLACK_PIECE_MESH));
        loader->get()->board_texture_small = std::make_shared<TextureData>(path(BOARD_TEXTURE_SMALL), true);
        loader->get()->board_paint_diffuse_texture_small = std::make_shared<TextureData>(path(BOARD_PAINT_TEXTURE_SMALL), true);
        loader->get()->white_piece_texture_small = std::make_shared<TextureData>(path(WHITE_PIECE_TEXTURE_SMALL), true);
        loader->get()->black_piece_texture_small = std::make_shared<TextureData>(path(BLACK_PIECE_TEXTURE_SMALL), true);
        loader->get()->white_indicator_texture = std::make_shared<TextureData>(path(WHITE_INDICATOR_TEXTURE), true);
        loader->get()->black_indicator_texture = std::make_shared<TextureData>(path(BLACK_INDICATOR_TEXTURE), true);
        loader->get()->skybox_px_texture = std::make_shared<TextureData>(path(SKYBOX_PX_TEXTURE), false);
        loader->get()->skybox_nx_texture = std::make_shared<TextureData>(path(SKYBOX_NX_TEXTURE), false);
